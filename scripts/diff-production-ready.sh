...
FORUM_MAX_POSTS_PER_LINKED_TOPIC="${FORUM_MAX_POSTS_PER_LINKED_TOPIC:-1000}"
FORUM_MAX_POSTS_PER_UPDATES_TOPIC="${FORUM_MAX_POSTS_PER_UPDATES_TOPIC:-20}"
FORUM_MAX_CATEGORY_PAGES="${FORUM_MAX_CATEGORY_PAGES:-25}"
FORUM_FETCH_DELAY_MS="${FORUM_FETCH_DELAY_MS:-500}"
ENABLE_HYBRID_CORPUS_REFRESH="${ENABLE_HYBRID_CORPUS_REFRESH:-true}"
...
npm run infra:diff -- \
  ...
  -c forumMaxPostsPerLinkedTopic="$FORUM_MAX_POSTS_PER_LINKED_TOPIC" \
  -c forumMaxPostsPerUpdatesTopic="$FORUM_MAX_POSTS_PER_UPDATES_TOPIC" \
  -c forumMaxCategoryPages="$FORUM_MAX_CATEGORY_PAGES" \
  -c forumFetchDelayMs="$FORUM_FETCH_DELAY_MS" \
  -c enableHybridCorpusRefresh="$ENABLE_HYBRID_CORPUS_REFRESH" \
  "$@"
